...
let allowCrossDomain = function(req, res, next) {
    const allowOrigin = envConfig[envConfig.environmentName].allowOrigin || req.headers.origin || "*";
    res.header("Access-Control-Allow-Origin", allowOrigin);
    res.header("Access-Control-Allow-Credentials", true);
    res.header("Access-Control-Allow-Headers", "X-Requested-With, Content-Type, Authentication, Authorization");
...
router.post('/login', (req, res) => {
    request.post({
        ...
        if (JSON.parse(body).id) {
            ...
            return res.json({ tokenid: JSON.parse(body).id, userid: JSON.parse(body).userId, user: JSON.parse(body).user });
...
router.get('/user', (req, res) => {
    request.get({
        url: `${API_URL}/users/` + req.session.authUserId.userid + `?access_token=` + req.session.authTokenId.tokenid
    }, function(error, response, body) {
        if (res.status(200)) {
